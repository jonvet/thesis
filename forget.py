...
class ForgetThought_model(Skipthought_model):
    ...
    def create_ft(self, path, num_classes, step):
        ...
        self.y = tf.placeholder(tf.float32, [None, num_classes], 'labels')
        self.logits = tf.contrib.layers.fully_connected(
                    self.encoded_sentences, num_classes, activation_fn=None,
                    scope='encoder')
    ...
    def forget(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, len(train), model.para.batch_size):
                ...
                labels = np.ones([model.para.batch_size, 7])/7
...
model.create_ft(path = MODEL_PATH, num_classes = 7, step = step)
model.para.batch_size = 10
model.para.learning_rate = 0.0001
model.forget(epochs = 10)
